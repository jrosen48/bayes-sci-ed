...
\EJ{I have doubts about the ``Trust me, I'm a Bayesian'' part of the title, also because ``Trust'' now occurs twice. Maybe simply ``Building Trust in Science Through a Bayesian Perspective on Probability and Uncertainty''?}
\MK{Ok with the new title - I liked the sound of the trust me thing but utlimatley it may not be really on point!}
\EJ{I also uploaded my bibtex file, ``referenties.bib''. I could not get it to work, but you could copy the relevant references into your own bibtex file. Actually, I also managed to make the initial bibtex setup not work, and I have absolutely no clue as to how this happened.}
...
\EJ{Yeah so as a hard-nosed determinist I still have an issue with ``inherent'' uncertainty. If we don't sort this out we could resolve it through a footnote (e.g., ``EJ Wagenmakers would like to note that he is a determinist who believes that the concepts of probability and uncertainty belongs wholly to the mind \parencite{Jevons18741913}, with the possible exception of the strange world of quantum mechanics.''} 
\MK{I got rid of the inherent part.}

\EJ{It would be good to go over the reference list in detail; in particular, I noticed several ``bayes'' that should be ``Bayes'' (the B needs to be enclosed in brackets so it displays in upper case)}
...
\JR{may cut these depending on what E.J. adds below}
\EJ{OK I added a bunch of stuff. Feel free to ignore, cut, rewrite, integrate with other materials etc. Here is where my stuff starts...}

The famous French astronomer and mathematician Pierre-Simon Laplace argued that Bayes' rule quantifies how rational observers ought to change their opinion in light of new evidence; in this sense, Laplace believed that Bayes' rule was \emph{common sense expressed in numbers}. We now showcase several insights that follow directly from Bayes' rule. In our earlier exposition of Bayes' rule we already emphasized the idea that hypotheses gain credibility when they predict the data well, and lose credibility when they predict new data poorly \parencite{WagenmakersEtAl2016CD}. This idea generalizes what the mathematician George Polya termed the ``Fundamental Inductive Pattern'':
\begin{quotation}
